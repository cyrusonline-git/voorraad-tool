...
<form method="get" class="card mb-3"><div class="card-body d-flex flex-wrap align-items-end gap-2">
    <input type="text" name="q" value="{{ request('q') }}" class="form-control form-control-sm" style="max-width:260px" placeholder="contract, project, depot, subgroep, aanvrager">
...
<div class="card"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
    <thead><tr><th>Datum</th><th>Contract / project</th><th>Aan depot</th><th>Namens</th><th>Aangevraagd</th><th>Aanvrager</th><th>Status</th><th></th></tr></thead>
    <tbody>
    @forelse($aanvragen as $a)
        <tr>
            ...
            <td><span class="badge bg-secondary">{{ $a->aantal_machines }} stuks</span> <span class="small text-muted">{{ collect($a->machines)->map(fn ($r) => ($r['aantal'] ?? 1).'× '.$r['subgroep_nr'])->take(3)->implode(', ') }}{{ count($a->machines ?? []) > 3 ? '…' : '' }}</span></td>
